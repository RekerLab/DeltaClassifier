...
def cross_validation_y_shuffle(x, relation, y, prop, model, k=10, seed=1, demilitarization=-1, only_equals=False): # provide option to cross validate with x and y instead of file
  kf = KFold(n_splits=k, random_state=seed, shuffle=True)
  preds = []
  vals  = []

  for train, test in kf.split(x):
      y[train] = np.random.permutation(y[train].values) # Adversarial Attack - Y-shuffling
...
def cross_validation_file_y_shuffle(data_path, prop, model, k=10, seed=1, demilitarization=-1, only_equals=False): # Cross-validate from a file
...
################################################
### Y-Shuffling Adversarial Attack - 1x10 CV ###
################################################
...
training_approaches = [(0.1, False)] # Set for training with our standard demilitarization
...
for prop in properties:
  for training_approach in training_approaches:
    for model in models:
      ...
      results = cross_validation_file_y_shuffle(data_path=dataset, prop = prop, model=model, k=10, seed = 1, demilitarization = training_approach[0], only_equals = training_approach[1])

      pd.DataFrame(results).to_csv("{}_{}_Y-Shuffled_{}.csv".format(prop, model.name(training_approach[0], training_approach[1]), 1), index=False)
      # If you .T the dataframe, then the first column is ground truth, the second is predictions

      df = pd.read_csv("{}_{}_Y-Shuffled_{}.csv".format(prop, model.name(training_approach[0], training_approach[1]), 1)).T
      ...
      results.to_csv("{}-DeltaClassifier-{}-Y-Shuffled-Metrics.csv".format(prop, model.name(training_approach[0], training_approach[1])), index = False)
